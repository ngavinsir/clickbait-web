<div class={`flex items-center ${wrapperClass(data.label.value)} rounded-lg shadow-xl px-4 py-4`}>
    <span class="value text-left w-full leading-none">{data.article.headline}</span>
    <svg class="cursor-pointer ml-4 group" on:click={() => dispatch('delete', data.label.id)} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
